...
-- Adds a domain tag so Fire Ops and Medical EMR documents can be governed
-- separately.  Existing rows default to 'fire_ops'.
--
-- Valid values (enforced by application, not a PG constraint, so future
-- domains can be added without a schema change):
--   fire_ops    — fire operations, equipment manuals, tactical SOPs
--   medical_emr — Emergency Medical Response, first aid, CPR, EMR protocols

ALTER TABLE corpus_documents
  ADD COLUMN IF NOT EXISTS domain TEXT NOT NULL DEFAULT 'fire_ops';
